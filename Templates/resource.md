<%*
let name = await tp.system.prompt("Name of this resource?");
let id = name.toLowerCase().replace(/\s+/g, "-");
let isFuel = await tp.system.suggester(["Yes", "No"], [true, false], false, "Is this resource used as fuel?");
let fuelTier = "";
let burnTime = "";
if (isFuel) {
  fuelTier = Number(await tp.system.prompt("Fuel tier (higher = hotter/better)?", "1"));
  burnTime = Number(await tp.system.prompt("Burn time (seconds)?", "10"));
}
-%>
---
id: <% id %>
tier: <% Number(await tp.system.prompt("Tier?", "1")) %>
category: <% (await tp.system.prompt("Category (e.g. metal, wood, stone, food)?")).toLowerCase() %>
